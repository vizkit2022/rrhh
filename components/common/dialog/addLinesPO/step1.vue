...
// Emits
const emit = defineEmits(["closeModal", "changeStep","updatedTable", "updateMetrics"]);
...
const { t } = useI18n();
const module = "modules.outcomes.pages.oc.modals.addLines.step1";
const buttons = "modules.outcomes.pages.oc.modals.addLines.step1.buttons";

// Mounted
onMounted(async () => {
  try {
    globalStore.loading = true;
    selectedLines.value = outcomesStore.formAddLines.lines || [];
...
const addLines = async () => {
    try {
        ...
        for (const line of selectedLines.value) {
          ...
          line.currency = mov.currency;
          idsCurrencies.add(line.currency);
        }

        const currencyPO = outcomesStore.outcome.currency.default;
        outcomesStore.formAddLines.currencies = [];

        // Flujo para cuando es la misma moneda todo - AQUI TERMINA
        if(idsCurrencies.length === 1 && idsCurrencies[0] === currencyPO) {
            let newLines = {
            ...
            const { data } = await outcomesStore.addLineOutcomeMany(newLines);
            ...
            // Flujo para diferentes monedas
            let resp = {};
    
            outcomesStore.formAddLines.currency = currencyPO;
    
            if(organizationStore?.organization?.currency?._id === outcomesStore?.outcome?.currency?.default?._id) {
              resp = await globalStore.getMyCurrencies();
            } else {
              const currencyBase = outcomesStore?.formAddLines?.currency?._id || null;
              if(currencyBase) {
            ...
            if(resp) {
              outcomesStore.formAddLines.currencies = resp.currencies || [];
              if(resp.currency) outcomesStore.formAddLines.currencies.unshift(resp.currency);
            }
    
            outcomesStore.formAddLines.lines = selectedLines.value;
            outcomesStore.formAddLines.type = outcomesStore.outcome.type;
            outcomesStore.formAddLines.outcome = outcomesStore.outcome._id;
            outcomesStore.formAddLines.documentType = outcomesStore.outcome.documentType;
            outcomesStore.formAddLines.numbers = { ...outcomesStore.outcome.numbers }
...
<template>
  <div class="step2">
    ...
    <div class="step2__body">
      <span class="label" v-text="t(module + '.mov.label')"></span>
      <u-search
        v-model="movement"
        :placeholder="t(module + '.mov.placeholder')"
      ...
      />
      <span class="label" v-text="t(module + '.lines.label')"></span>
      <div class="list">
        <span
          v-if="movements.length === 0"
          v-text="t(module + '.lines.noData')"
          class="noData"
        ></span>
        <div
          class="card"
        ...
        >
          <div class="card__header">
            <button class="card__header-text" @click="expandMov(m)">
              ...
              <span v-text="t(module + '.lines.text1')"></span>
            </button>
