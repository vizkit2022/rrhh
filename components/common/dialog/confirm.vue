...
const confirmationTextComputed = computed(() => {
  return formatTitle(
    props.confirmationText || t("confirm.modal.confirmationText")
  );
});
...
const deleteBtnText = computed(() => {
  return props.customTextBtnDelete.trim() === '' ? t('confirm.modal.deleteButton') : props.customTextBtnDelete;
});
  ...
  <div class="dialog" v-if="props.showModal">
    ...
    <div class="dialog__container">
      <div class="dialog__icon">
        <img :src="`/img/trash${globalStore.isDark ? '-dark' : ''}.gif`" alt="Delete Icon" />
      </div>
      <div class="dialog__label body-l-sb">
        <span>{{ props.title || t("confirm.modal.title") }}</span>
      </div>
      <div class="dialog__description">
        <span class="u u-info-circle"></span>
        <!-- Utilizar t() para la descripción si no se proporciona -->
        <span class="body-m-r">{{
          props.description || t("confirm.modal.description")
        }}</span>
      </div>
      <div class="dialog__input" v-if="props.showInput">
        <span class="body-s-sb">
          {{ $t("confirm.pre") }}
          <strong style="color: var(--danger-text-default); font-weight: 800">
            {{ confirmationTextComputed }}</strong
          >
          {{ t("confirm.despues").replace('NAME_BTN_MAIN', deleteBtnText) }}
        </span>
        <u-input
          :placeholder="
            t('confirm.modal.placeholder', {
              confirmationText: confirmationTextComputed,
            })
          "
          size="l"
          class="inputModify"
      ...
      <div class="dialog__actions">
        <u-button
          ...
          :text="t('confirm.modal.cancelButton')"
          class="btnMainModify"
          :disabled="loading"
          @click="closeModalAction"
        />
