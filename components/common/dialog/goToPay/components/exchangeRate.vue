...
const displayedCurrencies = computed(() => {
  ...
  const source = paymentsStore.formGoToPay.currency
    ? paymentsStore.formGoToPay.currencies.filter(
        (c) => c._id !== paymentsStore.formGoToPay.currency._id,
      )
    : currencies;
...
const selectedCurrency = async (currency, c) => {
  ...
  paymentsStore.formGoToPay.loadings.currencies = true;
  const resp = await globalStore.getMyCurrencies(false, currency._id);
  paymentsStore.formGoToPay.loadings.currencies = false;
