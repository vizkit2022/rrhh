...
const handleFileChange = async (event) => {
  ...
  try {
    paymentsStore.payment.processing = true;
    globalStore.loading = true;
    const resp = await paymentsStore.uploadPaymentFile(paymentsStore.payment._id, file);
    if(resp.success) await paymentsStore.getPaymentById(paymentsStore.payment._id);
  } catch (error) {
    console.error(error);
  } finally {
    globalStore.loading = false;
    event.target.value = null;
    paymentsStore.payment.processing = false;
  }
};
...
try {
    ...
    const fileToDelete = {
      fileName: file?.url,
      payment: paymentsStore?.payment?._id
    };
    // Pedir endpoint para eliminar archivos de un pago por su id
    const resp = await paymentsStore.deletePaymentFile(fileToDelete);
    if(resp.success) await paymentsStore.getPaymentById(paymentsStore.payment._id)
  } catch (error) {
    console.error(error);
  } finally {
...
const confirmFile = (file) => {
  if(paymentsStore.payment.status !== 'voided') {
    paymentsStore.payment.files.forEach(f => {
    ...
    <div class="file__table">
      <div class="file__table-headerContent">
        <div class="file__table-header">
          <div class="col"><span  v-text="t(module + '.texts.name')"></span></div>
          <div class="col"><span  v-text="t(module + '.texts.size')"></span></div>
          <div class="col"><span  v-text="t(module + '.texts.date')"></span></div>
          <div class="col"><span></span></div>
        </div>
      </div>
      <div class="file__table-item" v-for="file in paymentsStore.payment.files" :key="file">
        <div class="file__table-info" :style="`transform: translateX(${file.confirm ? '-100%' : '0'});`">
          ...
          <div class="col actions">
            <button @click="seeFile(file)"><span v-text="t(module + '.texts.see')"></span></button>
            <button @click="confirmFile(file)" :disabled="paymentsStore.payment.status === 'voided'"><span class="u u-close"></span></button>
          </div>
        </div>
        <div class="file__table-confirm" :style="`transform: translateX(${file.confirm ? '-100%' : '0'});`">
          <span class="question" v-text="t(module + '.texts.confirm')"></span>
          <u-button :text="t(module + '.texts.no')" @click="file.confirm = false" size="s" :color="color" :colorHover="colorHover" :colorActive="colorHover"/>
          <u-button :text="t(module + '.texts.yes')" @click="deleteFile(file)" size="s" />
        </div>
      </div>
    </div>
    <label v-if="paymentsStore.payment.status !== 'voided'">
      <u-button class="payButton" type="outlined" :text="t(module + '.texts.addFiles')" icon="attach" size="s" @click="triggerFileInput" />   
