...
<template>
  <div class="containerModal">
    <div class="container-modal__header">
      <span>
        {{ $t("topay.header." + props.mode) + " - " + $t("topay.subheader.step5") }}
      </span>
      <u-button-icon
        icon="close"
        class="btnIconModify"
    ...
    <div class="containerModal__content">
      ...
      <div class="container_header">
          ...
          <div class="container_provedor">
            <!-- Información del proveedor -->
            <u-avatar
              :user="{
              src:
                  mode != 'Searching'
                    ? paymentsStore.toPay.supplier?.imgUrl || ''
                    : '',
              }"
              :size="68"
            />
            <div class="supplier_info">
              <span class="legal-name">
                {{ paymentsStore.toPay.supplier.data.legalName || "-" }}
                <template v-if="paymentsStore.toPay.supplier.isContact">
                  <img
              ...
              <span class="nickName">
                {{ paymentsStore.toPay.supplier.data.nickName || "-" }}
              </span>
              <span class="address">
                {{ paymentsStore.toPay.supplier.email || "-" }}
              </span>
            </div>
            <!-- Botones -->
          </div>
      ...
      <div class="form_titulo">
        <span class="titulo_formulario">{{ $t("topay.subheader.step4") }}</span>
    ...
    <div class="containerModal__footer">
      <u-button
        :text="'Cancelar'"
        @click="$bus.$emit('updatedStepPay', 4)"
      />
      <u-button
        :text="'Siguiente'"
        @click="$bus.$emit('CreateToPay')"
...
.address {
  ...
  line-height: 20px;
  text-align: left;
  color: var(--Primary-Text-Default, #20a789);
}
