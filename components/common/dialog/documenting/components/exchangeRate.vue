...
const displayedCurrencies = computed(() => {
  const currencies = outcomesStore.formDocumenting.currencies || [];

  const source = outcomesStore.formDocumenting.currency
    ? outcomesStore.formDocumenting.currencies.filter(
        (c) => c._id !== outcomesStore.formDocumenting.currency._id,
      )
    : currencies;
...
const selectedCurrency = async (currency, c) => {
  ...
  outcomesStore.formDocumenting.currencyName = obj.name[lang.value];

  outcomesStore.formDocumenting.loadings.currencies = true;
  const resp = await globalStore.getMyCurrencies(false, currency._id);
  outcomesStore.formDocumenting.loadings.currencies = false;
