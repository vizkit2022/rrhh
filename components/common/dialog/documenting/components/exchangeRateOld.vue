...
const handleClickOutside = (event) => {
  if (outcomesStore.outcome.type === "OC") return;
...
const selectedCurrency = async (currency, c) => {
  ...
  try {
    if (currency?._id === outcomesStore.formDocumenting.currency._id) return;
    ...
    outcomesStore.formDocumenting.currency = obj;

    outcomesStore.formDocumenting.money = obj._id;
    outcomesStore.formDocumenting.moneyName = obj.name[lang.value];
    ...
    let resp = {};
    if (
      Object.keys(incomesStore.income).length > 0 &&
      currency._id === incomesStore.income.currency.default._id
    ) {
      resp = await globalStore.getMyCurrencies(
        false,
        incomesStore?.income?.currency?.default?._id,
      );
      salesStore.loadings.currencies = false;
    } else {
      resp = await globalStore.getMyCurrencies(false, currency._id);
      salesStore.loadings.currencies = false;
    }

    // igualmente no es necesario cambiar el valor
    outcomesStore.formDocumenting.currencies = [];
    if (resp) {
      ...
      outcomesStore.formDocumenting.currencies = resp.currencies.map((c) => {
        return {
          ...c,
      ...
      outcomesStore.formDocumenting.othersCurrency = resp.currencies.map((c) => {
        return {
          ...c,
          valueToday: normalizeToNumber(c.value),
          valueManual: normalizeToNumber(c.value),
    ...
    <div class="exhangeRate__list" v-show="!tempCurrency">
      <div class="exchangeRate__list-header">
        ...
        <u-button
          v-else-if="
            showAllCurrencies &&
            outcomesStore.formDocumenting.othersCurrency.length > 0
          "
          text="Tasa de cambio"
          icon="undo"
          :disabled="disabledOptions"
      ...
      <button
        ...
        :class="{
          selected:
            currency._id === outcomesStore.formDocumenting.currency?._id,
          disabled: disabledOptions,
        }"
        @click="selectedCurrency(currency, c)"
      >
        <div class="tag">
      ...
      <span
        v-if="!outcomesStore.formDocumenting.currencies.length"
        class="noData"
      >
        Sin monedas
      </span>
