...
const { t } = useI18n();
const module = "modules.sales.modals.charge.step1";
...
onBeforeMount(() => {
  if (!paymentsStore.formDeposits.initializedTable) {
    ...
    paymentsStore?.formDeposits?.lines?.forEach((l) => {
      ...
      const totalUnCharged = l.numbers.unCharged.number;

      // Guardando valores iniciales para el recalculo de moneda nueva a base
      l.numbers.unChargedBase = totalUnCharged;
...
const defaultCurrency = computed(() => {
  ...
  return currency || {};
  // return paymentsStore.formDeposits.currency  || {};
});

const baseCurrency = computed(() => paymentsStore.formDeposits.currencyInitial); // Moneda inicial

const invalidNumbers = computed(() => {
  const sePaso = paymentsStore.formDeposits.lines.some(
    (line) => line.numbers.amountDeposit.number > line.numbers.unCharged.number,
  );
...
const onPercentageInput = (event, p) => {
  ...
  const total = line.numbers.unCharged.number || 0;
...
const onPercentageBlur = (p) => {
  ...
  // REMOVIDO: Validar rango 0 - 100
  // Solo validamos que sea >= 0
  ...
  const line = paymentsStore.formDeposits.lines[p];
  const total = line.numbers.unCharged.number;
...
// Recalcula las tasas de cambio de la tabla
...
//       const oldUnCharged = line.numbers.unCharged.number || 0;
//       const oldTotal = line.numbers.total.number || 0;

//       let convertedUnCharged;
//       let convertedTotal;
...
//         unChargedInBase = oldUnCharged * oldRate;
...
//         convertedUnCharged = unChargedInBase;
...
//         convertedUnCharged = unChargedInBase / newRate;
//         convertedTotal = totalInBase / newRate;
//       }

//       // 3. Actualizar unCharged y total con los valores convertidos
...
// watch(() => paymentsStore.formDeposits.currency,
      ...
      <div class="step2__body-table">
        <div class="step2__body-table-header">
          ...
          <div class="col right">
            <span
              v-text="
                `${t(`${module}.table.cols.col4`)} [${paymentsStore.formDeposits.currency.code}]`
              "
            ></span>
          </div>
          <div class="col right">
          ...
          <div class="col right">
            <span
              v-text="
                `${t(`${module}.table.cols.col5`)} [${paymentsStore.formDeposits.currency.code}]`
              "
            ></span>
          </div>
        ...
        >
          ...
          <div class="col right">
            <span
              ...
              :title="item?.numbers?.totalPostCreditAndDebit?.number"
            ></span>
          </div>
          <div class="col right">
            <span
              v-text="item?.numbers?.unCharged?.value"
              :title="item?.numbers?.unCharged?.number"
...
.step2__body-table-item {
  display: grid;
  width: 100%;
  grid-template-columns: 140px minmax(180px, 1fr) 170px 185px 80px 220px 50px;
}
