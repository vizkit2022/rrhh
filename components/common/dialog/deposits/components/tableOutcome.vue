...
const { t } = useI18n();
const module = "modules.outcomes.pages.oc.modals.deposits.step1";
...
onBeforeMount(() => {
  if (!paymentsStore.formDeposits.initializedTable) {
    paymentsStore.formDeposits.supplier =
      paymentsStore?.formDeposits?.lines?.[0]?.supplier;
    paymentsStore?.formDeposits?.lines?.forEach((l) => {

      l.numbers.unpaidBase = l.numbers.unpaid.number;
      l.numbers.totalBase = l.numbers.total.number;

      l.numbers.percentage = "0";
      l.numbers.amountDeposit = formatData(0, defaultCurrency.value);
    });
...
const invalidNumbers = computed(() => {
  const sePaso = paymentsStore.formDeposits.lines.some(
    (line) => line.numbers.amountDeposit.number > line.numbers.total.number
  );
...
const onPercentageInput = (event, p) => {
  ...
  const total = line.numbers.total.number || 0;
...
const onPercentageBlur = (p) => {
  ...
  const line = paymentsStore.formDeposits.lines[p];
  const total = line.numbers.total.number;
...
//       const oldUnCharged = line.numbers.unpaid.number || 0;
...
//       line.numbers.unpaid = formatData(convertedUnCharged, newCurrency);
    ...
    <div class="step2__body">
      <div class="step2__body-table">
        <div class="step2__body-table-header">
          ...
          <div class="col right">
            <span v-text="t(module + '.table.headers.toDeposit')"></span>
          </div>
        ...
        >
          ...
          <div class="col left name">
            ...
            <span
              ...
              v-text="t(module + '.multipleIncomes')"
            ></span>
            <a
              v-else
              class="income"
          ...
          <div class="col right">
            <input
              ...
              "
              :class="
                item.numbers.amountDeposit.number > item.numbers.total.number
                  ? 'error'
                  : ''
              "
            />
