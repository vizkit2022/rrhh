...
// Constants
const { t } = useI18n();
const module = "modules.outcomes.pages.oc.sections.information";

// Computed
const creator = computed(() => ({
  name: outcomesStore?.outcome?.creator?.user?.data?.legalName || "-",
  src: outcomesStore?.outcome?.creator?.user?.imgUrl,
}));
const voided = computed(() => ({
  name: outcomesStore?.outcome?.voidedData?.voidedBy?.data?.legalName || "-",
  src: outcomesStore?.outcome?.voidedData?.voidedBy?.imgUrl,
  date: outcomesStore?.outcome?.voidedData?.voidedDate ? transformedDate(outcomesStore?.outcome?.voidedData?.voidedDate) : "-"
}));
      ...
      <div class="information__card-form">
        ...
        <div class="information__group">
          ...
          <div class="avatar">
            ...
            <span class="information__group-value" v-text="creator.name"></span>
          </div>
        </div>
        <!-- Anulado por -->
        <div class="information__group">
          ...
          ></span>
          <div class="avatar">
            <u-avatar :size="24" :user="voided" v-if="voided.src" />
            <span class="information__group-value" v-text="voided.name"></span>
          </div>
        </div>
        <!-- Fecha de creación -->
        <div class="information__group">
          ...
          ></span>
          <span
            class="information__group-value"
            v-text="transformedDate(outcomesStore?.outcome?.createdAt)"
          ></span>
        </div>
        ...
        <div class="information__group">
          ...
          ></span>
          <span class="information__group-value" v-text="voided.date"></span>
        </div>
      </div>
    </div>
...
.information {
  ...
  gap: 20px;
  overflow: auto;
  width: v-bind("globalStore.sliderExpand ? 'calc(100vw - 318px)' : 'calc(100vw - 160px)'");
  height: 100%;
  padding-right: 4px;
