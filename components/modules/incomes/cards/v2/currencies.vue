...
const currencyList = JSON.parse(
  JSON.stringify([
    {
      currency: incomesStore.income.currency.default,
      valueManual: 1,
      valueToday: 1,
    },
    ...incomesStore.income.currency.others,
  ])
);
...
const selectCurrency = (pos) => {
  ...
  const prop =
    grillaStore.configDropdownCell.type === "incomes" ? "price" : "budget";
  const newValue = currencies.value[pos];
...
onMounted(() => {
  const prop =
    grillaStore.configDropdownCell.type === "incomes"
      ...
      : "currencyBudget";
  let idCurrency = null;
  if (grillaStore.configDropdownCell.line?.data?.[prop]) {
    idCurrency = grillaStore.configDropdownCell.line?.data[prop]._id;
  } else {
...
<template>
  <div class="dropdown">
    ...
    <div class="dropdown__list">
      <button
        v-for="(objC, c) in currencies"
        :key="c"
        :class="{ btnCurrency: true, selected: objC.selected }"
