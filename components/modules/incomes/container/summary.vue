...
const selectedLines = computed(() => {
  if(linesStore?.linesSurcharges) return linesStore.linesSurcharges.filter((l) => l.selected);
  return [];
});
...
const deleteLines = async () => {
  if(permissionsStore.myPermissions.surcharges__delete) {
    cleanLines();
  }
  ...
  if (selectedLines.value.length) {
    try {
      ...
      calculatingValues.value = calculatingValuesLabels.delete[globalStore.lang];
...
const addLines = async (type = "line", pos = 0) => {
  if(permissionsStore.myPermissions.surcharges__add) {
    try {
      const newLine = {
        income: idIncome,
        isCut: type === "cut",
      ...
      if (type === "cut") {
        isInteractable.value = true;
        calculatingValues.value = calculatingValuesLabels.realTime[globalStore.lang];
...
const selectedLine = (line, pos) => {};

const changeName = async (line, pos, updateTotals = true) => {
  try {
    if (line._id) {
      isInteractable.value = true;
      calculatingValues.value = calculatingValuesLabels.updated[globalStore.lang];
      ...
      isInteractable.value = true;
      calculatingValues.value = calculatingValuesLabels.create[globalStore.lang];
      incomesStore.loadings.metrics = true;
...
const changePercentage = async (line, i) => {
  if(latinMode && (line?.difference?.isDifference || line?.finalValue?.isFinalValue)) {
    ...
    try {
      incomesStore.loadings.metrics = true;
      isInteractable.value = true;
      calculatingValues.value = calculatingValuesLabels.calc[globalStore.lang];
...
const startEditing = (event, index, state, line) => {
  if(latinMode && (line?.difference?.isDifference || line?.finalValue?.isFinalValue)) {
    return null;
  } else {
    const name = event.target.name;
...
const permissionsColumns = () => {
  if(permissionsStore.myPermissions.income_grid_col_incomes) {
    propsLine.value.push({key: "sumPrice", label: "Total Venta", col: "col1"})
  };
  if(permissionsStore.myPermissions.income_grid_col_budget) {
    propsLine.value.push({key: "sumBudget", label: "C. Presupuestado", col: "col2"})
  };
  if(permissionsStore.myPermissions.income_grid_col_cost) {
    propsLine.value.push({key: "cost", label: "Costos", col: "col3"})
    propsLine.value.push({key: "toSpend", label: "Por Gastar", col: "col3"})
    propsLine.value.push({key: "efc", label: "EFC", col: "col3"})
  };
  if(permissionsStore.myPermissions.income_grid_col_projected) {
    propsLine.value.push({key: "projectedUtility", label: "Utilidad", col: "col4"})
  };
};

const getBorderClass = (pos) => {
  if(latinMode) return "border-bottom"
  const totals = linesStore.linesSurcharges.length - 1;
  return pos === totals ? "no-border" : "border-bottom";
};
const selectValueInput = (event, line) => {
  if(latinMode && (line?.difference?.isDifference || line?.finalValue?.isFinalValue)) {
    return null;
  } else {
    event.target.select();
...
const onInputOnlyNumbers = (event, line, prop) => {
...
const searchLines = async (line, pos) => {
  if(latinMode && (line?.difference?.isDifference || line?.finalValue?.isFinalValue)) {
    ...
    if (!!toSearch) {
      ...
      await linesStore.searchForTaxesNames(toSearch, idIncome);
...
const handleKeyUpInput = (event, line) => {
  if(latinMode && (line?.difference?.isDifference || line?.finalValue?.isFinalValue)) {
    return null;
  } else {
    const key = event.key;
...
const openDifference = async (line) => {
  if(line?.difference?.state === 'closed') {
    const finalTotal = linesStore.linesSurcharges.find(l => l.finalValue.isFinalValue);
    const body = {
      ...line,
  ...
  <div class="container" id="containerSummary">
    ...
    <div class="table">
      <div class="table__header">
        ...
        <div :class="`table__header-col ${prop.col}`" v-for="prop in propsLine" :key="prop.key">
      ...
      <div class="table__groups">
        ...
        <div class="table__header-col" v-for="prop in propsLine" :key="prop.key">
          <span class="amount truncateText">
            {{ incomesStore.totalIncome[prop.key].value }}
          </span>
        </div>
      </div>
      <div class="table__containerGroups">
        ...
        >
          ...
          <div class="table__containerGroups-col" v-for="prop in propsLine" :key="prop.key">
            <span class="truncateText">{{ group.numbers[prop.key].value }}</span>
          </div>
        </div>
      </div>
      <div class="table__subheader">
        <div class="table__subheader-actions">
          ...
          <button
            @click="deleteLines"
            class="btnIcon"
            :disabled="!permissionsStore.myPermissions.surcharges__delete || isInteractable || selectedLines.length === 0"
          >
          ...
          <button
            class="btnIcon"
            @click="addLines('line')"
            :disabled="!permissionsStore.myPermissions.surcharges__add || isInteractable"
          >
        ...
        <div class="table__header-col" v-for="n in propsLine" :key="n">
          <span></span>
        </div>
      </div>
      <div
        :class="`table__item ${line.isCut ? ' main' : 'second'} ${
          'table__item-' + i } ${line?.finalValue?.isFinalValue ? 'finalValue' : ''}`"
      ...
      >
        <div class="table__item-actions cell" :class="getBorderClass(i)">
          <u-checkbox
            ...
            @click="selectedLine(line, i)"
            :height="16"
            :disabled="isInteractable"
            v-if="!(latinMode && (line?.difference?.isDifference || line?.finalValue?.isFinalValue))"
          />
          <button
            class="btnIcon"
            :disabled="!permissionsStore.myPermissions.surcharges__add || isInteractable"
          ...
          <button
            class="btnIcon"
            :disabled="!permissionsStore.myPermissions.surcharges__add || isInteractable"
            v-if="!line.isCut && !(latinMode && (line?.difference?.isDifference || line?.finalValue?.isFinalValue))"
        ...
        <div
          :class="`table__item-name cell ${getBorderClass(i)} ${latinMode && (line?.difference?.isDifference || line?.finalValue?.isFinalValue) ? 'single' : ''}`"
          :id="`row-${i}`"
        >
          <input
            type="text"
            ...
            v-model="line.name"
            autocomplete="off"
            :disabled="!permissionsStore.myPermissions.surcharges__add || (latinMode && (line?.difference?.isDifference || line?.finalValue?.isFinalValue)) || isInteractable"
            @input="searchLines(line, i)"
            @keydown="handleKeyUpInput($event, line)"
          />
          <button
            class="btnIcon info"
            @click="showModalInfo(i, line)"
            v-if="!permissionsStore.myPermissions.surcharges__add || !(latinMode && (line?.difference?.isDifference || line?.finalValue?.isFinalValue))"
            :disabled="isInteractable || ['', null, undefined].includes(line._id)"
          >
          ...
          <input
            ...
            @focus="startEditing($event, i, true, line)"
            @blur="startEditing($event, i, false, line)"
            @keypress="keyPressAction($event, 'percentage', line)"
            @click="selectValueInput($event, line)"
            v-if="!(latinMode && (line?.difference?.isDifference || line?.finalValue?.isFinalValue))"
            :disabled="!permissionsStore.myPermissions.surcharges__add || isInteractable || ['', null, undefined].includes(line._id)"
          />
        </div>
        <div
          :class="`table__item-col cell ${latinMode && line?.difference?.isDifference && prop.key === 'sumPrice' ? 'double' : ''} ${getBorderClass(i)}`"
          v-for="prop in propsLine"
          :key="prop.key"
        >
          <button 
            v-if="latinMode && line?.difference?.isDifference && prop.key === 'sumPrice'"
            @click="openDifference(line)"
            :disabled="!permissionsStore.myPermissions.surcharges__add || calculatingValues || line?.difference?.state === 'open'">
            <span :class="`u u-${line?.difference?.state === 'closed' ? 'locked' : 'unlocked'}`"></span>
          </button>
          <input
            type="text"
            class="inputCustom"
            :name="prop.key"
            v-model="line.numbers[prop.key].value"
            @change="changeValue($event, line, i, prop.key)"
            :disabled="!permissionsStore.myPermissions.surcharges__add || (latinMode && (line?.difference?.isDifference || line?.finalValue?.isFinalValue) && prop.key === 'sumBudget') || calculatingValues || isInputDisabled(prop.key, line)"
            @focus="onFocusInput($event, prop.key, line)"
            @blur="onBlurInput($event, prop.key, line, i)"
            @input="onInputOnlyNumbers($event, line, prop.key)"
          />
          <!-- @keypress="$event.preventDefault()" -->
        </div>
      </div>
      <div class="table__footer" v-if="!latinMode">
        ...
        <div class="table__footer-col" v-for="prop in propsLine" :key="prop">
          <span class="amount truncateText">{{
            incomesStore?.totalGrossIncome?.[prop.key].value
          }}</span>
        </div>
      </div>
    </div>
...
.second.finalValue div {
  background-color: var(--neutral-surface-softer);
}

.second:not(.finalValue):hover div.cell {
  transition: 0.3s cubic-bezier(0.075, 0.82, 0.165, 1);
...
.table__item-col.cell.double {
  padding-left: 12px;
  display: grid;
  grid-template-columns: auto 1fr;
}
.table__item-col.cell.double button:disabled {
  cursor: not-allowed;
}
.table__item-col.cell.double button {
  color: var(--neutral-text-caption)
}
.table__item-col.cell.double button:hover {
  color: var(--primary-text-default);
}
...
.table__item-name:not(.single) input[name="name"] {
  width: calc(100% - 120px);
}
.table__item-name.single input[name="name"] {
  width: 100%;
}
.table__item-name.single input[name="name"]:disabled {
  color: var(--neutral-text-body) !important;
  opacity: 1 !important;
}
.table__item.finalValue input {
