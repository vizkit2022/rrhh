...
//TRANSLATIONS
const { t } = useI18n();
const module = "modules.incomes.pages.settings.tabs.globals";
...
const nameType = computed(() => {
  return {
    constant: t(`${module}.table.types.constant`),
    variable: t(`${module}.table.types.variable`),
    text: t(`${module}.table.types.text`),
  };
});

const optionSelectType = computed(() => [
  { label: t(`${module}.table.types.constant`), value: "constant" },
  { label: t(`${module}.table.types.variable`), value: "variable" },
  { label: t(`${module}.table.types.text`), value: "text" },
]);
...
async function addNewGroup(newName, printGroup) {
  const body = {
    name: newName,
  ...
  try {
    ...
    currentGroup.value =
      incomesStore?.configuration?.globals?.groups?.length - 1;
...
async function changeGroup(index) {
  ...
  try {
    ...
    await incomesStore.getGlobals(incomesStore.income?._id);

    incomesStore.configuration.globals.globalsByGroup =
      incomesStore.configuration.globals.allGlobals.filter(
        (item) =>
          item.groupVariable ===
    ...
    console.error("Error al cambiar de grupo:", e);
  } finally {
    globalStore.loading = false;
  }
}
...
async function editGroup(newName, printGroup) {
  const body = {
    _id: groupId?.value,
    name: newName,
    printGroup: printGroup,
...
async function deleteGroup() {
  try {
    ...
    // si el grupo de globals no esta vacio obtener los globals de ese grupo
...
function deleteTemporalGlobal() {
  ...
  // Usar nextTick para asegurar que los cambios se apliquen en el siguiente ciclo
  nextTick(() => {
    // Encontrar el índice del elemento temporal en globalsByGroup
    const temporalIndex =
      incomesStore.configuration.globals.globalsByGroup.findIndex(
        (item) => item.isTemporal === true
      );

    if (temporalIndex !== -1) {
      // Remover específicamente por índice
      incomesStore.configuration.globals.globalsByGroup.splice(
        temporalIndex,
        1
      );
    }
...
function filterCalculationInput(event, item) {
  //CASO GLOBAL TIPO TEXT

  if (item.type === "text") {
    if (event.target.value.length > MAX_WORD) {
      event.target.value = event.target.value.slice(0, MAX_WORD);
      return;
    }
    item.calculation.displayFormula = event.target.value;
    calculationValueLocal(item, event.target.value);
    return;
  }

  // CASO GLOBAL TIPO CONSTANTES Y VARIABLES
...
function calculationValueLocal(item, expression) {
  ...
  if (!expression || expression.trim() === "") {
    ...
    item.calculation.displayFormula = "";
    item.value = 0;
    item.calculation.value = 0;
    return;
  }

  if (item.type === "text") {
    item.calculation.formula = "";
    item.calculation.displayFormula = expression;
    item.value = 0;
    item.calculation.value = 0;
    return;
  }
  ...
  try {
    ...
    const resultRounded = parseFloat(result.toFixed(4));
...
//Funcion para cuando es tipo TEXT en calculation value
async function calculationTypeText(item, expression) {
  const MAX_WORD = 5;

  // VALIDACION DE TIPO TEXTO
  if (expression.split(" ").length > MAX_WORD) {
    item.value = "Error";
    return;
  }
  ...
  if (item.isTemporal) {
    globalStore.loading = true;
    item.calculation.formula = expression;
    item.calculation.dependencies = [];
    item.value = 0;
...
async function calculationValue(item, expression) {
  ...
  // CALCULATION DE TIPO TEXTO
  if (item.type === "text") {
    calculationTypeText(item, expression);
    return;
  }

  // CALCULATION DE TIPO NUMERICO (CONTANTE O VARIABLE)
  if (!expression || expression.trim() === "") {
    item.calculation.formula = "";
    item.value = 0;
    return;
  ...
  try {
    ...
    if (!item.isTemporal) {
      console.log("actualiza calculo no temporal");
...
async function updatedNameGlobal(item, value) {
  ...
  try {
    ...
    // push temporal globals al final
    incomesStore?.configuration?.globals?.globalsByGroup?.push(
      ...temporalGlobals.value
    );
  } catch (e) {
...
async function updatedAbbreviationGlobal(item, value) {
  ...
  try {
    ...
    // verificar si la abreviacion del temporal esta repetida al actualizar otro global
    const temporalGlobalGroup =
      incomesStore.configuration.globals.globalsByGroup.find(
        (global) => global.isTemporal === true
      );

    if (temporalGlobalGroup) {
      checkAbreviationLocal(
        temporalGlobalGroup.abbreviation,
        incomesStore.income?._id,
        temporalGlobalGroup
      );
    }
  } catch (e) {
...
async function updatedTypeGlobal(data, item) {
  // Si cambia de 'text' a 'constant' o 'variable', resetear propiedades
  const isChangingFromText =
    item.type === "text" &&
    (data.value === "constant" || data.value === "variable");

  if (item.isTemporal) {
    item.type = data.value;

    // Resetear propiedades si cambia de text
    if (isChangingFromText) {
      item.calculation.displayFormula = "0";
      item.calculation.formula = "0";
      item.calculation.dependencies = [];
      item.value = 0;
    }
    ...
    const body = {
      ...
      abbreviation: item.abbreviation,
      calculation: {
        displayFormula: isChangingFromText
          ? "0"
          : item.calculation.displayFormula,
        formula: isChangingFromText ? "0" : item.calculation.formula,
        dependencies: isChangingFromText ? [] : item.calculation.dependencies,
      },
      value: isChangingFromText ? 0 : item.value,
      type: data.value,
      income: incomesStore.income?._id,
      ...(item.unit !== null && item.unit !== "Sin unidad"
        ? { unit: item.unit._id }
    ...
    try {
      ...
      // Actualizar el item local antes de la petición
      if (isChangingFromText) {
        item.calculation.displayFormula = "0";
        item.calculation.formula = "0";
        item.calculation.dependencies = [];
    ...
    <div class="globals__body">
      ...
      >
        ...
        <template v-slot:type="item">
          <u-select
            class="selectGlobals"
            :model-value="nameType[item.item.type]"
        ...
        <template v-slot:calculation="item">
          <u-input
            ...
            :placeholder="
              item.item.type === 'constant'
                ? '10 * (5 + 2)'
                : item.item.type === 'variable'
                ? '10 * (C + D)'
                : item.item.type === 'text'
                ? t(`${module}.table.inputs.typeText.placeholder`)
                : ''
            "
            @blur="
              () => {
        ...
        <template v-slot:value="item">
          <span
            ...
            >{{
              item.item.type !== "text"
                ? item?.item?.value || item?.item?.calculation.value
                : item?.item?.calculation?.displayFormula
            }}</span
          >
        </template>
