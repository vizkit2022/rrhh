...
const buttons = computed(() => [...buttonsOrigin.value]);
...
const save = async (button, pos) => {
  if (!button.disabled) {
    ...
    if(actions[button.prop]) {
      await actions[button.prop]()
    } else {
      await actions["defaultProp"]();
    }
  ...
  <div class="modal">
    ...
    <div class="modal__title">
      <span class="body-l-sb">{{
        t(module + ".subtitle", { type: namesState[globalStore.tag] })
      }}</span>
    </div>
    ...
    <div class="modal__content">
      <u-button-process
        ...
        :disabled="button.disabled || button.loading || saving"
        @click="save(button, b)"
        size="l"
      />
    </div>
    ...
    <div class="modal__announcement">
      ...
      <p class="">
        {{ t(module + ".textAnnouncement") }}
      </p>
    </div>

    <div class="modal__actions">
      <u-button
        :text="t(module + '.buttons.configDoc')"
        type="text"
        icon="settings"
        :disabled="saving"
