...
const addSurchargeLine = async (surcharge, type = "") => {
  ...
  const prop = "surchargesLinesBudget";
  ...
  if (type === "budget") {
    ...
    if (!line.changeBudgetTax) {
      ...
      linesStore.switchesBudget = JSON.parse(
        JSON.stringify(linesStore.switchesBudget),
      );
    }
  }
...
const getValueLine = (surcharge, index) => {
  const line = linesStore.line;
  const prop = "surchargesLinesBudget";
  const propTotal = "totalLineBudget";
  const propSwitch = "switchesBudget";
...
<template>
  <div class="containerDialog__form">
    <div class="mainLabel">
      <span>{{ t(module + ".subtext4") }}</span>
      <span class="amount">
        {{ linesStore.line?.numbers?.sumBudget?.value }}
      </span>
    </div>
    <template
      ...
      <div class="normalLabel" v-if="!surcharge.isCut">
        <div class="box">
          <u-switch
            ...
            @click="addSurchargeLine(surcharge, 'budget')"
            :disabled="
              surcharge.surchargeVariables.sumBudget.state !== 'toLines' ||
        ...
        <span
          class="amount"
          v-if="surcharge.surchargeVariables.sumBudget.state === 'toLines'"
        >
