...
// EMITS
const emit = defineEmits(["closeModal", "lockModal", "updatedMetrics"]);
...
const errors = ref({});
const dataCreatedAt = infoProject?.createdAt || "";
const form = ref({
  projectId: infoProject?._id || null,
  state: props.state,
  projectName: infoProject?.name || "",
  businessArea: {
    name: infoProject?.businessArea?.name || "",
    id: infoProject?.businessArea?._id || null,
  },
  executive: {
    name: infoProject?.executive?.data?.legalName || "",
    id: infoProject?.executive?.contact?._id || null,
    email: infoProject?.executive?.email || "",
    img: infoProject?.executive?.imgUrl || null,
  },
  client: {
    name: infoProject?.client?.data?.legalName || "",
    id: infoProject?.client?.contact?._id || null,
    email: infoProject?.client?.email || "",
    img: infoProject?.client?.imgUrl || null,
  },
  contact: {
    name: infoProject?.contact?.data?.legalName || "",
    id: infoProject?.contact?.contact?._id || null,
    email: infoProject?.contact?.email || "",
    img: infoProject?.contact?.imgUrl || null,
  },
  completionDate: infoProject?.completionDate,
  assignmentDate: infoProject?.assignmentDate,
  currency: {},
  currencyName: "",
});
const results = ref([]);
...
const save = async () => {
  if (isValidForm()) {
    ...
    try {
      await incomesStore.updatedProject(body);
      await incomesStore.getProjectBasicData(form.value.projectId);
      if (change.value) emit("updatedMetrics");
...
const isValidForm = () => {
  ...
  console.log(
    "form",
    form.value.businessArea.name.trim(),
    form.value.businessArea.id,
  );
...
// Buscar Miembros
const searchMembers = debounce(async (toSearch) => {
...
//watch permissions
// const isUpdateName = ref(false);
// const isUpdateCliente = ref(false);
//   watch(
//     () => permissionsStore.profilePermissionsUserSession,
//     (newVal) => {
//       const permissions = newVal?.permissions;
//       isUpdateName.value = permissions == null
//         ? true
//         : permissions.every((permission) => permission.code !== "edit_project_name");

//       isUpdateCliente.value = permissions == null
//         ? true
//         : permissions.every((permission) => permission.code !== "edit_project_client");
...
onMounted(async () => {
  try {
    // await middlewarePermisssionActivation('updatedProject');
...
const loadCurrencies = async () => {
  ...
  form.value.currency =
    incomesStore?.currentProject?.currency ||
...
onUnmounted(() => {
  ...
  // permissionsStore.permissionsActivated.isEditProjectsClient = false;
  // permissionsStore.permissionsActivated.isEditProjectsName = false;
});

const handleEscapeKey = (event) => {
  if (event.key === "Escape" || event.key === "Esc") {
    if (!saving.value) emit("closeModal");
  }
};
</script>
...
<template>
  <div class="createModal">
      ...
      <template v-if="tabSelected === 0">
        <div class="createModal__content-group">
          ...
          <u-input
            ...
            :disabled="
              saving ||
              !permissionsStore.myPermissions?.income_edit_project_name
            "
          />
        </div>
    ...
    <div class="createModal__actions">
      ...
      />
      <u-button
        :text="$t(`createProject.buttons.${saving ? 'saving' : 'save'}`)"
        class="mainBtnModify"
        :disabled="saving"
        @click="save"
      />
