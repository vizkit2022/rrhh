...
const toggleExpanded = (item, index) => {
  ...
  // Obtenemos la posición real del elemento en el array en caso de que el array haya sido modificado
  const realIndex = contentTable.value.findIndex(i => i.id === item.id);

  if (item.expanded) {
    // Asegurarse de que childrens es una copia para evitar mutaciones indeseadas
    const childrens = item.childrens.map(child => ({
      ...child,
      isChild: true
    }));
    // Inserta los hijos en contentTable.value después del Item actual
    contentTable.value.splice(realIndex + 1, 0, ...childrens);
  } else {
    // Contar y remover todos los elementos seguidos que tienen la propiedad isChild en true
    let count = 0;
    for (let i = realIndex + 1; i < contentTable.value.length && contentTable.value[i].isChild; i++) {
      count++;
    }
    // Remover los hijos de contentTable.value
    contentTable.value.splice(realIndex + 1, count);
  }
};
...
<template>
  <div v-bind="containerProps" class="table">
    ...
    <template v-else-if="contentTable.length">
      <div class="table__item" v-for="(item, i) in contentTable" :key="i" :class="item.isChild ? 'div-child': ''">
        <button
        ...
        >
          ...
          <div class="colCustom" v-if="col.type === 'text_group'">
            <u-button-icon
              class="expandCustom"
              icon="chevron-down"
              type="text"
              v-show="item.isChild ? false : true"
              :style="`transform: rotate(${item.expanded ? 0 : -90}deg)`"
              @action-btn="toggleExpanded(item, i)"
            />
            <span class="truncateText" :class="item.isChild ? 'item-child' : ''" v-text="item[col.property]"></span>
          </div>
          <span
            v-if="col.type === 'date'"
            v-text="getParseDate(item[col.property])"
          ></span>
          <div class="tag">
            <span
              v-if="col.type === 'tag'"
