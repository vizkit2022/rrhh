...
<template>
  <div class="container__info-box">
    <template v-if="props.user.roles">
      <span class="title">Roles</span>
      <template v-if="props.user.roles.length > 0">
        <div class="tagsBox">
          <u-tags
            v-for="user in props.user.roles"
            :key="user._id"
    ...
    <span class="title">Disponible en</span>
    <div class="items scroll" v-if="props.user.locations?.length">
      <div class="item" v-for="(location, l) in props.user.locations" :key="l">
        <div>
          <img
            :src="location.url ? location.url : 'https://www.expreso.info/files/2021-07/Vina%20del%20Mar.jpg'"
            alt="city"
          />
        </div>
        <div>
    ...
    <div v-else>
      <span class="msg">Desconocido</span>
    </div>
  </div>
</template>
