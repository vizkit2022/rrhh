  ...
  <div class="pay">
    <div class="table">
      <div class="table__header">
        <div class="cell"><span>{{ t(LABEL_KEY + ".texts.document") }}</span></div>
      ...
      <div
        class="table__item"
        v-for="line in bankingTransactionsStore?.dataDetailsFlowsSalesDoc?.lines"
        :key="line?._id"
      >
        <div class="cell truncateText">
          <span class="truncateText" v-text="line?.saleDocument?.correlative"></span>
        </div>
        <div class="cell">
          <a
            class="truncateText"
            :href="`/salesDocuments/${line?.saleDocument?._id}`"
            target="_blank"
            rel="noopener noreferrer"
            v-text="line?.saleDocument?.reference"
          ></a>
          <a
            class="truncateText"
            v-if="line?.saleDocument?.incomes?.length === 1"
            :href="`/incomesv2/project/${line?.saleDocument?.incomes?.[0]?.project?._id}/income/${line?.saleDocument?.incomes?.[0]?._id}`"
            target="_blank"
            rel="noopener noreferrer"
          >
            <span class="u u-open"></span>
            <span class="truncateText">{{
              line?.saleDocument?.incomes?.[0]?.name
            }}</span>
          </a>
          <span
            v-else
        ...
        <div class="cell truncateText">
          <span
            class="truncateText"
            v-text="line?.numbers?.total?.value"
            :title="line?.numbers?.total?.number"
          ></span>
        </div>
      </div>
    </div>
    <div class="table__footer">
      ...
      <div class="cell truncateText">
        <span
          class="truncateText"
          v-text="
            bankingTransactionsStore?.dataDetailsFlowsSalesDoc?.numbers?.total?.value
          "
          :title="
            bankingTransactionsStore?.dataDetailsFlowsSalesDoc?.numbers?.total?.number
          "
        ></span>
      </div>
    </div>
