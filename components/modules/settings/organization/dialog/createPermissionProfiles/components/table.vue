...
const changeState = (
  ...
  if (permission.area.area_code === "projects") validityProject(config);
    ...
    const pos = props.pos;
    organizationStore.permissionsProfile.create.permissions[pos].groups[
      g
    ].subcategories[s].permissions[p][state2] =
      !organizationStore.permissionsProfile.create.permissions[pos].groups[g]
...
const validityProject = (config) => {
  ...
  codes.forEach((code) => {
    const perm = organizationStore.permissionsProfile.create.permissions[
      pos
