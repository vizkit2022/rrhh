...
const KEY_LABLES = "modules.organizations.settings.members.modal.permissions.sections.labels";
...
const isActive = computed(() => {
  const CODE = props.permission.area_code === "globals" ? "globals" : (props.permission.area_code + "_global");
  const globals = permissionStore.permissionsUser.find(p => p.area_code === "globals");
  if(CODE === "globals") return true;
  if(globals) {
    const perm = globals.groups[0].subcategories[0].permissions.find(p => p.code === CODE);
...
<template>
  <div class="permission__card">
    <button :class="`permission__card-header ${(expand && isActive) ? 'expanded' : ''}`" :disabled="!isActive" @click="toggleExpand">
