...
const changeState = (
  ...
  if (permission.area.area_code === "projects") validityProject(config);
    ...
    const pos = props.pos;
    organizationStore.permissionsProfile.edit.permissions[pos].groups[
      g
    ].subcategories[s].permissions[p][state2] =
      !organizationStore.permissionsProfile.edit.permissions[pos].groups[g]
        .subcategories[s].permissions[p][state];
  }
};
// PROJECTS
const validityProject = (config) => {
  ...
  codes.forEach((code) => {
    const perm = organizationStore.permissionsProfile.edit.permissions[
      pos
    ].groups[g].subcategories[s].permissions.find((perm) => perm.code === code);
    if (perm) {
          ...
          <div class="permissionTable__item">
            ...
            <div class="permissionTable__item-col">
              <u-radio
                ...
                "
                :disabled="
                  !organizationStore.permissionsProfile.edit.actions.edit
                "
              />
            </div>
            <div class="permissionTable__item-col">
